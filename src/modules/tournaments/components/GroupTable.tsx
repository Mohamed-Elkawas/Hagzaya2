    ...
    >
        ...
        <span
          className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-black ${
            qualifies
              ...
              : 'bg-[#f0f2f0] text-on-surface-variant'
          }`}
        >
          {rank}
        </span>
      ...
      <td className="px-3 py-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-[#191c1c] truncate max-w-30">
            {team.name}
          </span>
          {qualifies && (
      ...
      {/* Stats */}
      <td className="px-3 py-3 text-center text-sm text-on-surface-variant">{team.played}</td>
      <td className="px-3 py-3 text-center text-sm font-semibold text-emerald-600">{team.won}</td>
      <td className="px-3 py-3 text-center text-sm text-on-surface-variant">{team.drawn}</td>
      <td className="px-3 py-3 text-center text-sm font-semibold text-red-500">{team.lost}</td>
      <td className="px-3 py-3 text-center text-sm text-on-surface-variant">{team.goalsFor}</td>
      <td className="px-3 py-3 text-center text-sm text-on-surface-variant">{team.goalsAgainst}</td>
...
export function GroupTable({ group }: GroupTableProps) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#e1e3e1] shadow-sm overflow-hidden">
      {/* Group Header */}
      <div className="px-4 py-3 bg-linear-to-r from-[#003d12] to-primary flex items-center gap-2">
        ...
        <table className="w-full min-w-140">
          <thead>
            <tr className="border-b border-[#e1e3e1] bg-[#f6f8f7]">
              <th className="px-3 py-2.5 text-center text-[11px] font-bold text-on-surface-variant/60 uppercase tracking-wider w-10">
                #
              </th>
              <th className="px-3 py-2.5 text-start text-[11px] font-bold text-on-surface-variant/60 uppercase tracking-wider">
                الفريق
              </th>
              <th className="px-3 py-2.5 text-center text-[11px] font-bold text-on-surface-variant/60 uppercase tracking-wider">
                لعب
              </th>
              <th className="px-3 py-2.5 text-center text-[11px] font-bold text-on-surface-variant/60 uppercase tracking-wider">
                فاز
      ...
      <div className="px-4 py-2 border-t border-[#f0f2f0] flex items-center gap-4 bg-[#f6f8f7]">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-on-surface-variant/70 font-medium">متأهل للدور القادم</span>
        </div>
      </div>
    </div>
  );
