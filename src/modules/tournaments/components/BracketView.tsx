    ...
    >
        ...
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <span
            className={`text-xs truncate ${
              winnerIsA ? 'font-black text-primary' : 'font-semibold text-[#191c1c]'
            }`}
          >
            {match.teamA?.name ?? 'TBD'}
          </span>
        </div>
        <span className={`text-xs font-black ms-2 ${winnerIsA ? 'text-primary' : 'text-[#191c1c]'}`}>
          {scoreA ?? '-'}
        </span>
      </div>
      ...
      >
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <span
            className={`text-xs truncate ${
              winnerIsB ? 'font-black text-primary' : 'font-semibold text-[#191c1c]'
            }`}
          >
            {match.teamB?.name ?? 'TBD'}
          </span>
        </div>
        <span className={`text-xs font-black ms-2 ${winnerIsB ? 'text-primary' : 'text-[#191c1c]'}`}>
          {scoreB ?? '-'}
        </span>
      </div>
    </div>
...
function BracketColumn({ stageName, matches, isLast }: BracketColumnProps) {
  return (
    <div className="flex flex-col items-center shrink-0">
      {/* Column Header */}
      <div className="mb-4 px-3 py-1.5 bg-primary text-white text-xs font-bold rounded-full shadow">
...
function ChampionSlot({ matches }: { matches: Match[] }) {
  ...
  return (
    <div className="flex flex-col items-center ms-4">
      ...
      <div className="w-32 flex flex-col items-center gap-2 bg-linear-to-b from-yellow-50 to-amber-50 border-2 border-yellow-300 rounded-2xl p-4 shadow-xl">
